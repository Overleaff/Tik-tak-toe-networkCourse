        ...
        if (receive > 0)
        {
            if (strlen(buffer) > 0)
            {
                ...
                else if (strcmp(command, "join") == 0)
                {
                    ...
                    for (int i = 0; i < MAX_ROOMS; i++)
                    {
                        if (rooms[i])
                        {
                            if (rooms[i]->uid == number)
                            {
                                ...
                                bzero(buffer, BUFFER_SZ);
                                printf("%s enter room number: %i\n", cli->name, number);
                                sprintf(buffer, "[SERVER] '%s' enter the room\n", cli->name);
                                send_message(buffer, rooms[i]->player1->uid);

                                bzero(buffer, BUFFER_SZ);
                                sprintf(buffer, "[SERVER] you already entered the room number: %i\n", number);
                                send_message(buffer, cli->uid);
                                break;
                            }
                        }
                    ...
                    if (researched == 0)
                    {
                        sprintf(buffer, "[SERVER] can not find the room number %i\n", number);
                        send_message(buffer, cli->uid);
                    }
                }
...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        {
            printf("Maximum of clients are connected, Connection rejected");
            close(connfd);
            continue;
        }
