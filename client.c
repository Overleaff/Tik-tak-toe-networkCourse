...
void recv_msg(int sockfd, char * msg)
{
    ...
    if (n < 0 || n != 3)
     herror("ERROR reading message from server socket.");
...
int recv_int(int sockfd)
{
    ...
    if (n < 0 || n != sizeof(int)) 
       herror("ERROR reading int from server socket");
...
void write_server_int(int sockfd, int msg)
{
    ...
    if (n < 0)
       herror("ERROR writing int to server socket");
...
int connect_to_server(char * hostname, int portno)
{
    ...
    if (sockfd < 0) 
       herror("ERROR opening socket for server.");
   ...
   if (connect(sockfd, (struct sockaddr *) &serv_addr, sizeof(serv_addr)) < 0) 
       herror("ERROR connecting to server");
